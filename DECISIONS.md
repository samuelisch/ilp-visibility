...
The shipped v1 UI was generic Tailwind grays + system font. Ratified overhaul direction: **warm & calm** — a cream/ink palette + clay accent + calm-teal data colors as Tailwind v4 `@theme` tokens; type pairing **Fraunces** (display) + **Hanken Grotesk** (body, `tabular-nums` figures), self-hosted via Fontsource; the landing table becomes a soft **`PolicyCard`** grid; shared UI primitives (`Card`/`Pill`/`Button`/`Field`/`SegmentedControl`/`StatCard`) that also encapsulate Tailwind (the "extract components, not class strings" move); **restrained motion** (`motion` lib — one staggered reveal, hover lift, gentle chart draw-on, respecting `prefers-reduced-motion`). The detail page is re-themed in place. **Implemented & complete 2026-06-17** (`fe/closed/ui-overhaul-*`; landing e2e updated for the card redesign, commits `29516da`/`519f740`). Design source `~/.claude/plans/now-i-want-to-sprightly-wave.md`.

## 040 — Premium nature is `premiumAllocationType`, not `paymentTermYears === null` — 2026-06-19

Surfaced from GREAT Life Advantage 4 (id 72), a **regular-premium whole-of-life (open-ended)** ILP that was being labelled "Single premium" on the landing card. Root cause: `paymentTermYears: null` is **overloaded** — of the 43 records (out of 204) with a null term, only 30 are true single premium; the other **13 are regular open-ended** (8 `recurring` + 5 `term`). So `null` cannot mean "single."

**The canonical single-vs-regular signal is `PolicyAccount.premiumAllocationType`**, with one rule everywhere: a policy is single iff **`accounts.every(a => a.premiumAllocationType === 'single')`** — the rule the illustration engine already uses (`runIllustration`). `paymentTermYears` stays orthogonal: it means _premium-paying duration_ (a number = fixed MIP; `null` = no fixed term). The three premium states: **single** → "Single premium"; **regular + N** → "N-year MIP"; **regular + null** → "Open-ended".

**Resolution (no schema change):** expose `policyAccounts.premiumAllocationType` in the list `select` (it lived only in the detail tree, so the landing card couldn't see it) and derive nature on the FE via one shared `isSinglePremium` helper. Verified empirically that no policy has more than one `single` account (every SP product is one account at `allocationFrom` year 1), so `every(...)` is unambiguous; the multi-tranche / single-premium-top-up case that would complicate it is already deferred from v1.

**Rejected:** (a) encoding single as `paymentTermYears = 1` — re-introduces a magic value (still renders "1-year MIP" without special-casing) and creates a _second_ source of truth for single-ness that can drift from `premiumAllocationType`; (b) denormalising a `premiumType` column onto `Policy` — duplicates a per-account, derivable value (drift risk) and costs a migration + reseed for no gain. Both repeat the overloading/duplication this decision removes. Tickets: `be/open/premium-nature-select-shape.md`, `fe/open/premium-nature-labels.md`.

## 041 — Sub-fund layer: provider-level menu, projected on AMC (not TER) — 2026-07-14

Building out the fund-layer cost DEC 028 deferred. A 10-provider agent fan-out captured, per provider, the available ILP **sub-funds + per-fund management fees**, in `pdfs/extraction-plan/subfunds/` (one `<provider>.md` per provider + `findings/`).

- **Provider-level, not policy-level.** Sub-funds belong to the **provider**, not individual policies. The FE fund-selector reads the policy's provider and lets the user self-select — dropping the messy, largely-inferred policy→fund mapping and keeping the ★★★-confirmed provider menus. Data model: `Provider → SubFund`, not a Policy↔SubFund many-to-many.
- **Project on AMC, not TER.** The 40-year illustration drags on the fund's **AMC** (Annual Management Charge) — a stated, contractual, _forward_ rate. **TER** (Total Expense Ratio) is the fuller all-in cost but is _backward-looking, variable, and distorted on small funds_ (e.g. NTUC Global Eco TER 19.5%, an artifact of a tiny asset base), so it's unsafe as a 40-year constant. Rule: **AMC as the base; TER only as fallback where AMC is absent, outliers capped.** Store `{ amc, ter, asOf, basis }` per fund; present as _indicative_. TER-as-current-cost is a separate, deferred use case.
- This fund-layer cost sits **on top of** the policy-level charges already in the engine; together = the "complete fee picture."

**Status:** data gathered 2026-07-14; not yet in the Prisma schema. Known gap: Etiqa & Tokio Marine `fee: not disclosed` lines are wrong — AMC lives in their Product Highlights Sheets (independent Codex check, gpt-5.5); re-gather pending. See `pdfs/extraction-plan/subfunds/README.md`.
