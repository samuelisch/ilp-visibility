...
## 023 — Singlife seed data: bonuses excluded as a package, 102% allocation as bonus — 2026-06-11

Seeded all 10 Singlife variants (Legacy Invest: SP/3Y/5Y/10Y premium terms; Savvy Invest II: 3/5/10-Year Fixed, 5/10/20-Year Flexible). Both mapping and independent verification agents extracted every rate table cleanly — zero numeric discrepancies. Decisions ratified:

**Legacy Invest's four bonuses are excluded, despite being deterministic and disclosed.** Welcome Bonus (2% SP / 4–6% 3Y / 8–10% 5Y / 10–12% 10Y of first-year premium, premium-band dependent), Special Booster (1.75%/2.50% of premiums paid, at end of PPT), Loyalty Bonus (0.30% p.a. of account value post-PPT), Maturity Bonus (3–5% of account value). The schema has no bonus entity and the existing precedent (HSBC Flexi Protector's 102% allocation, PRUVantage Assure's premium-size credit, all Welcome Bonuses to date) excludes bonuses uniformly. Consequence accepted: Legacy Invest's modeled net cost is overstated more than any peer's — the bonus package is unusually large because it offsets the steep front-loaded Administrative Charge (3.0–3.5% p.a. of AV for the first 4–10 years, the product's only modeled fee). If a bonus entity ever enters scope, Legacy Invest is the motivating product.

**Savvy Invest II 20-Year Flexible's 102% premium allocation in policy years 11–20 is treated as an excluded bonus allocation** (`premiumChargePercentage: 0.00`), same reasoning as Flexi Protector in Decision 022. The PDF allocates 100% for monthly premiums 1–120, 102% for 121–240, 105% from 241. Only the 20-Year Flexible variant has a >100% phase inside its modeled payment window (paymentTermYears = MIP = 20); for all other variants it falls in the unmodeled post-MIP period.

Also notable, verified but requiring no new conventions: Legacy Invest has no Cost of Insurance at all (s4.8 "Not applicable" — death cover via the 101%-of-premiums floor, not a mortality charge); Savvy Invest II's Supplementary Charge (1.90% p.a. of AV) runs a fixed 10 policy years regardless of variant, outliving the 3F/5F payment terms; Fixed and Flexible variants share identical surrender schedules for the same duration (cell-level verified — they differ only in the out-of-scope partial-withdrawal table); paymentTermYears = MIP for Savvy Invest II per the Decision 022 convention (premiums payable whole-of-life); the year-7→8 surrender-rate drop from 40% to 20% in both 10-year Savvy columns is genuine, not an extraction error.

## 024 — FWD seed data: min()-fee chaining, single-account products, PPT sampling — 2026-06-11

Seeded all 16 FWD variants across 6 families (Invest First Horizon 20/25Y; Invest Flexi VII MIT-10; Invest First Summit PPT 10/15/20/25/30; Invest Flexi Elite 3-flexi/5-flexi; Invest Goal 1 SP; Invest First Max PPT 10/15/20/25/30). Both fan-outs (mapping + independent verification) found zero numeric discrepancies; the hardest tables (First Max's merged staircase grid, the 21-column Appendix A tables) were verified at word-coordinate level and against rendered page images. Decisions ratified:

**min()-of-two-bases fees are modeled as two chained rows that switch base at a frozen crossover year.** Summit's accumulation account charge is `lower of (1.5% p.a. × AUA value) or (0.7% × PPT × annualised premium p.a.)` — no schema construct expresses min(). Modeling only the AV leg would overstate the fee up to ~6× at long horizons (the cap leg, fixed in dollars, decays toward 0% of AV as the account grows — by year 30 under 8% the real charge is ~0.2% of AV vs the modeled 1.5%, a ~1.3 p.p. annual drag that could flip cross-provider rankings). Instead: row 1 = 1.5% `account_value` for years 1–(K−1); row 2 = (0.7 × PPT)% of `annual_premium`, perpetual from year K. K is the midpoint of the simulated crossover years under the 3% and 8% scenarios (fee-netted monthly recurrence, contributions years 3–PPT): cap leg from year 7/8/10/12/14 for PPT 10/15/20/25/30. Error is confined to ~2 years around the crossover, where the legs are nearly equal by definition. This extends Decision 022's materialize-the-formula stance; if min()-shaped fees recur (Etiqa/Tokio Marine), consider a fee-cap field instead.

**Single-account modeling for Horizon, Flexi VII, and Flexi Elite.** Unlike the HSBC Wealth Accelerate precedent, these PDFs route ALL regular premiums to the initial units account for the entire policy term; the accumulation units account's only inflow is out-of-scope top-ups. The AUA is omitted entirely (modeling it would force `allocationFromPolicyYear` to misroute premiums). Summit and First Max, by contrast, have a genuine 24-month split (IUA years 1–2, AUA from year 3) and are modeled with two accounts.

**Quinquennial PPT sampling for Summit and First Max.** Both PDFs offer every integer PPT from 10 to 30 (21 columns in their Appendix A tables). Seeded PPT 10/15/20/25/30 only — 5 files per family instead of 21.

**Recurring single premium (First Max) is top-up-equivalent and excluded.** Same 5% charge, same destination (base-layer AUA); the only difference is scheduling. Premium-increase "layers" (each with own accounts, clocks, and schedules) are likewise not modeled — base layer only, consistent with no-premium-increase scope.

**Flexi VII MIT = 10 is inferred, not stated.** The PDF never quantifies the MIT; surrender period (years 1–10), redemption window (years 3–10), and Loyalty Bonus from year 11 converge on 10. **Flexi Elite paymentTermYears = 10 (the MIT), not the flexi number** — "3/5 flexi" governs the shortfall-charge window and premium-flexibility rights, not the payment term.

**Bonuses excluded throughout** (Booster / Annual Premium / Loyalty / Contribution / Perpetual / Accumulation Bonuses), per Decision 023. Horizon's package is large (Booster up to 30% of first-two-years' premiums), so its modeled net cost is overstated more than most.

Also notable: Horizon and Flexi VII are the first real uses of `escalating_n` exactly as Decision 007 anticipated — the PDFs define "N = Policy Year during PPT/MIT, thereafter N = PPT/MIT", identical to the engine's min(year, paymentTermYears); Horizon's rate-phase boundary is asymmetric across variants (PPT 20 drops 3.8%→1.2% at year 10; PPT 25 drops 3.5%→1.0% at year 11) and is correct in the files. Summit, Goal 1, and First Max have no insurance/mortality charge at all (death benefit = 105% of policy value). Goal 1's surrender base is the single premium paid (`cumulative_premium_paid`), and its unit-count-based initial account charge ((1%/12) × units) is economically identical to 1% of account value. Summit and First Max share byte-identical Appendix A surrender tables.

## 025 — Etiqa seed data: Formula A/C landings, sliding-scale collapse, Flexi PPT, null-PPT semantics — 2026-06-11

Seeded 31 Etiqa variants across 11 product families (smart flex II 10/15/20Y; Smart Vista 10/15/20Y; flex wealth II and Wealth Purpose 3/5/10/15/20Y each; vista, flex prime II, flex pro, Prime Purpose at Flexi 3/Flexi 5/20Y each; Invest starter; Invest plus SP; Tiq Invest). All verified clean by independent re-extraction — zero numeric discrepancies. Etiqa is the provider whose Formula A and Formula C originally motivated `cumulative_premium_paid` (Decision 008) and `escalating_n` (Decision 007); both landed exactly as designed. The Shariah/Takaful twins (Smart Vista, vista, Prime Purpose, Wealth Purpose) are numerically identical to their conventional siblings — the Takaful sections only rename charges (insurance charge → Tabarru', shortfall → Gharamah); Wakalah is the agency principle, not a separate fee. Decisions ratified:

**Flexi variants: paymentTermYears = 10, not the flexi number** (8 files: vista/flex prime II/flex pro/Prime Purpose × Flexi 3/Flexi 5). The PDFs define the payment term itself as "ten (10) Years – Flexi 3 / Flexi 5"; the flexi number is only the premium-free-period entitlement (compulsory first 3/5 years, then 84/60 waiver months). Formula C's cap references the 10-year PPT — the worked example escalates the charge base through unpaid premium-free years and freezes it at year 10 — so escalating_n requires PPT = 10. The opposite resolution to FWD Flexi Elite (MIT), justified because here the PDF names the PPT explicitly.

**Formula A post-PPT sliding scale collapsed to 0.60% perpetual** (12 files). The scale is indexed by *number of annualised premiums paid* (= total paid ÷ annualised premium at inception, rounded down), not policy year. Under the on-time assumption exactly PPT premiums are paid, selecting the bottom rate — 0.60% in every table. A missed-premium history would lock in a permanently higher rate (up to 1.34%); not modeled. Ratified: annual premiums are always paid on time.

**flex wealth II / Wealth Purpose 3Y and 5Y: 2.60% genuinely perpetual.** Coordinate-verified single-row tables — no post-PPT step-down exists for these terms; 2.60% p.a. of cumulative premiums runs for the whole policy term (to age 100).

**paymentTermYears = null extended to "no committed payment term".** Invest starter is a whole-life regular premium product with no selectable term at all — premiums are payable indefinitely. Its 5-year surrender charge (7/7/6/6/5% of AV) is a de facto lock-in but lives entirely in the surrender fee schedule; paymentTermYears stays null because the engine reads the payment pattern from premiumAllocationType (Decision 012), not from this field. SCHEMA.md updated accordingly.

**Invest plus SP Representative Management Charge seeded at the 0.75% cap.** An optional adviser fee ("rate agreed between You and Your representative, subject to a maximum of 0.75% per annum", effectively 0 if never agreed) — worst-case negotiable-charge convention, GE Prestige Portfolio precedent.

**Dash PET Plus dropped; Tiq Invest kept.** Fee-identical twins (0.75% p.a. management charge, no insurance charge, no surrender fee), but Dash PET Plus is a yearly-renewable rider under a group policy owned by SingCash Pte. Ltd. (the user is not the policy owner), its termination is coupled to a Basic policy whose fees sit outside its Product Summary, its S$100/S$1,000,000 account-value bounds are not schema-representable, and the Singtel Dash distribution channel was announced for wind-down in 2025. It adds no fee information; the file was deleted.

Also notable: Tiq Invest and Invest plus SP have no insurance charge at all; Tiq Invest is a standalone plan (the research findings mislabeled it a rider — the PDF says otherwise); Invest plus SP's surrender base is the amount withdrawn (= account value on full surrender → `account_value`, not premium-based); Etiqa's separate Top-up Accounts (0% charges, top-up inflows only) are omitted across all products per Decision 024; the Start-up Bonus Recovery charge (clawback on premium decrease) is excluded because premium decreases are not modeled; all bonuses (Start-up/Special/Loyalty/Power-up/Policy Charge Refund/promotions) excluded per Decisions 023/024.

## 026 — Manulife seed data: first notional_premium use, MIP=lock-in semantics, InvestReady III SGD/USD split — 2026-06-11

Seeded 15 Manulife variants across 6 product families (Manulink Investor (II) SP — Cash/SRS + CPF; ManuInvest Duo MIP 10/15/20; Manulife InvestReady (III) — SGD, 4 MIPs; Manulife InvestReady Growth MIP 15/20; Manulife SmartRetire (V) Income and Sum, MIP 8/12 each). Mapped by per-product agent fan-out (in two batches after the Etiqa session-limit lesson) and passed the programmatic consistency check; the user opted to skip the independent verification fan-out this round. Provider legal name: Manulife (Singapore) Pte. Ltd. Decisions ratified:

**First `notional_premium` use in the dataset — InvestReady Growth.** Its Administrative Charge is `X%/12 × Value of Minimum Premium Payable`, where the base = annualised regular basic premiums payable up to the Flexi Start Date, accumulated at **6% p.a.** over the MIP — deterministic, independent of account value/market performance. Mapped to `feeType: "notional_premium"`, `notionalPercentage: 6.00`, `chargePercentage` = the admin rate (15Y: 2.18% in-MIP → 0.95% post-MIP; 20Y: 1.80% → 0.92%). Two ratified semantics set the precedent for this feeType: (a) the post-MIP phase stays on the **notional base** (it never reverts to account value — the PDF has one formula, only X% swaps); (b) the notional base **freezes** at the value accumulated to the Flexi Start Date over the MIP — it does not keep compounding at 6% afterwards. The engine (not yet built) must reproduce "accumulate premiums-to-Flexi-Start-Date at 6% over the MIP, then hold." If a future product contradicts the freeze reading, revisit.

**paymentTermYears = MIP = the surrender/lock-in period.** The MIP (Minimum Investment Period) is defined by the surrender-charge window: surrender charge runs through the MIP and is 0% after, so the point where the surrender fee vanishes *is* the MIP boundary. Premiums are contractually payable to age 99 (whole life), but that is not the meaningful fee term, so paymentTermYears = MIP per the Etiqa/FWD precedent. Subtlety: the Administrative Charge tier sometimes steps at a **fixed policy year**, not the MIP — ManuInvest Duo (5.0%→1.0%) and SmartRetire (2.50%→0.75%) both step at year 6 regardless of an 8/10/12/15/20-year MIP — so the admin charge is modeled with its own explicit `recurringLength` (5) rather than tying it to paymentTermYears. InvestReady (III) and Growth, by contrast, step exactly at the MIP boundary (recurringLength = MIP).

**InvestReady (III): two same-named summaries → two products distinguished by domicile; seed SGD now, defer USD.** WA_MIRP (SGD-only) and WA_MIR03 (multi-currency SGD+USD) share the policy name "Manulife InvestReady (III)". On every overlapping SGD variant their modeled fees (admin both phases + surrender schedule) are byte-identical. Resolution: WA_MIRP is the SGD product, seeded now (4 variants: 5Y Flexi4, 7Y Flexi5, 10Y Flexi3/5/8, 13Y Flexi10, domicile sgd); WA_MIR03 is the USD/multi-currency product, **deferred** until USD enters scope (keeps the v1 SGD-only exclusion intact). The 5Y Flexi1 and 6Y Flexi2 MIPs exist only in WA_MIR03, so they wait for the USD pass. Future direction: expand the `domicile` enum to represent multi-currency products properly rather than forcing one PDF to a single currency. InvestReady III post-MIP admin rate is keyed to MIP length (5/6/7Y → 1.00%, 10/13Y → 0.70%), resolving the findings' apparent "Flexi5 in both groups" ambiguity (it's 7Y-Flexi5 vs 10Y-Flexi5).

**Conditional S$5/month policy fee (InvestReady III 10Y & 13Y) kept out.** The PDF charges a flat S$5/month only when 1st-year annualised basic premium is in band (S$6,000–9,599 for 10Y Flexi3/5/8; S$3,600–9,599 for 13Y Flexi10); above ~S$9,600 it is S$0. With no premium-input UI yet and the persona plausibly above the band, the fee row was removed. The band thresholds are recorded here so it can be reinstated as a `flatFeeAmount` row with `minPremium`/`maxPremium` set (per Decision 008) when premium-band UI exists.

**SmartRetire (V) Income and Sum both kept.** Fee structures are identical (admin 2.50%→0.75% at year 6; same surrender schedules per MIP); they differ only in payout mechanic (monthly income drawdown vs lump-sum Target Retirement Sum). Unlike the dropped Dash PET Plus (a group-policy distribution artifact), these are two distinct retail products a consumer chooses between, so both are seeded with descriptions distinguishing them ("Income, …" / "Sum, …").

**Manulink Investor (II): Cash/SRS + CPF variants; zero ongoing fees.** Single-premium product with a 3% premium charge (Cash/SRS) and no ongoing admin/surrender/COI charge at all — its only modeled cost is the 3% upfront (sub-fund management fees are fund-layer, deferred to the future sub-fund data layer). The PDF accepts CPF-OA/CPF-SA but discloses no separate CPF premium-charge rate; a `cpfis` variant was created at **0% premium charge**, inferred from the cross-provider CPFIS convention (Decision 011: CPFIS premium charge is 0% on AIA/GE/Prudential/HSBC) — flagged as an inference subject to correction if Manulink's actual CPF figure differs.

Also ratified/excluded uniformly: Cost of Insurance everywhere (rate-table mortality on NAAR), including SmartRetire's two COI lines (death benefit + Waiver-of-Premium-on-TPD) and its "Refund of COI at Target Retirement Age" feature (which refunds an excluded charge → moot); Premium Shortfall Charges (missed-premium trigger, incl. InvestReady Growth's refundable variant); partial withdrawal charges and flexibility/life-stage-waiver tiers (out of v1 scope); top-up charges (0%/3%/5%, out of scope); S$100 change-of-life-insured fee (de-minimis); free fund switching (currently-nil); all bonuses (Welcome/Annual Premium/Loyalty/Step-up Booster) per Decisions 023/024/025. Flexi collapse: 10Y Flexi3/5/8 and 8Y Flexi3/5 collapsed to one file each (identical modeled fees). ManuInvest Duo's 5.0% p.a. Years 1–5 admin charge is the highest in the dataset.
