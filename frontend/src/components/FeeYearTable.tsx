...
export function FeeYearTable({ rows }: { rows: YearRow[] }) {
  ...
  return (
    <div data-testid="fee-year-table">
      <table className="tnum w-full text-left text-sm">
        <thead>
          <tr className="text-xs tracking-wide text-muted uppercase">
            ...
            <th className="py-1 pr-4 font-medium">Premium paid</th>
            <th className="py-1 pr-4 font-medium">Total fees</th>
            <th className="py-1 pr-4 font-medium">Gross value</th>
            <th className="py-1 pr-4 font-medium">Net value</th>
            <th className="py-1 font-medium">Earnings</th>
          </tr>
        </thead>
        <tbody>
          {slice.map((r) => (
