...
const rows: PolicyListItem[] = [
  { id: 1, name: 'Beta', description: 'single plan', domicile: 'sgd', paymentTermYears: null, sourceType: 'cash', provider: { name: 'AIA' } },
  { id: 2, name: 'Alpha', description: 'regular plan', domicile: 'sgd', paymentTermYears: 20, sourceType: 'cash', provider: { name: 'Manulife' } },
  { id: 3, name: 'Gamma', description: 'regular plan', domicile: 'usd', paymentTermYears: 10, sourceType: 'cash', provider: { name: 'AIA' } },
]
...
describe('selectVisibleRows — filter', () => {
  ...
  it('search matches description', () => {
    expect(
      selectVisibleRows(rows, { ...base, search: 'regular' }).map((r) => r.id).sort(),
    ).toEqual([2, 3])
  })
  it('provider filter restricts to one insurer', () => {
    expect(
      selectVisibleRows(rows, { ...base, providerFilter: 'AIA' }).map((r) => r.id).sort(),
    ).toEqual([1, 3])
  })
